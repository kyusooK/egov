    ...
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-actions>
            ...
            <v-btn
                ...
                color="primary"
                text
                @click="openCompleteReturn"
            >
                CompleteReturn
            </v-btn>
            <v-dialog v-model="completeReturnDiagram" width="500">
                <CompleteReturnCommand
                    @closeDialog="closeCompleteReturn"
	...
	async created() {
        ...
        methods: {
            ...
            async completeReturn(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['completereturn'].href), params)
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
            ...
            openCompleteReturn() {
                this.completeReturnDiagram = true;
            },
            closeCompleteReturn() {
                this.completeReturnDiagram = false;
            },
        },
    }
</script>
